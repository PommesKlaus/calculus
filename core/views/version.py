...
from core.models import Company, Year, Version
import pdb

class VersionListView(View):
    template_name = 'core/version_list.html'

    def get(self, request, *args, **kwargs):

        v = Version.objects.filter(year_id=kwargs['year_id'])

        # Feststellen, ob Jahr gelöscht werden kann
        for elem in v:
            if elem.version_py_version.count() > 0 or elem.version_tu_version.count() > 0:
