...
const BalanceSheetComponent = ({ LineItems, Differences, showAddDifferenceModal }) => (
    <div>
        <Button bsStyle="primary" onClick={() => showAddDifferenceModal(1)}>+ Neue HB/StB-Abweichung</Button>
        <table className="table table-hover table-condensed table-bordered balancesheet">
            <thead>
                ...
                <tbody>
                    <tr key={item.id}>
                        <td className="balance-sheet-icon-col subtotal"></td>
                        <td className="line-label subtotal">{item.name} (<a href="javascript:void(0)" className="nounderline" onClick={() => showAddDifferenceModal(item.id)}>+</a>)</td>
